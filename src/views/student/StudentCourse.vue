<template>
<div v-if="getCourseInfo" class="flex flex-grow flex-col p-8 relative">
    <div class=" flex w-full justify-center text-2xl">
        <div>Opis kursa</div>
    </div>
    <div class="mt-4"> <span class="underline"> Naziv kursa :</span> <span class="text-xl">{{getCourseInfo.name}}</span> </div>
    <div class="mt-4"> <span class="underline"> Cena kursa :</span> {{getCourseInfo.price}} din. </div>
    <div class="mt-4 leading-6"><span class="underline"> Opis kursa :</span> {{getCourseInfo.description}} </div>
    <div v-if="getCourseInfo.average_mark != 0" class="flex items-center mt-2">
    ...
    <div v-if="getApplyToCourse" class="flex shadow-2xl items-center flex-col mx-auto w-96 mt-24 border p-4 bg-blue-100 rounded-md">
...
export default class StudentCourse extends NotificationMessageMixin {
    ...
    created() {
        this.fetchCourseInfo({
            ...
            .then(() => {
				let averageMark : any = this.getCourseInfo && this.getCourseInfo?.average_mark ? this.getCourseInfo.average_mark : ''
                let width = Math.floor(averageMark / 5 * 230);
				setTimeout(() => {
					this.styleRate = `width:${width}px`;
					console.log(this.styleRate);
				}, 10);
	...
	@StudentStore.Getter
	public getApplyToCourse!: boolean | null;
